...
from CallerIDUi import Ui_Form
import sys
import signal
...
def SaveCall(phonenumber,name):
  ...
  con.execute("INSERT INTO phonecalls (name,phonenumber,calldate,Fax) VALUES ('"+name+"','"+phonenumber+"','"+str(calldate)+"',False)")
  ...
  print(text)
  TextEdit.insertPlainText(window,text)
  con.close()
...
def CreateHtml():
    ...
    with open('CallerID.html','w+') as File:
      File.write(html)
...
def CheckCalls():
  ...
  quit  - Quit the program\n"""
          ...
          if line2 != line and line !="\n":  
            ...
            TextEdit.insertPlainText(window,line)
          else:
            pass
        # last_work_time = now
...
if __name__ == "__main__":
    ...
    init()
    work_thread = threading.Thread(target=CheckCalls)
